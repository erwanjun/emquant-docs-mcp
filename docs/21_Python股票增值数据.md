...
[](#stk-get-industry-category-查询行业分类)
...
[](#stk-get-sector-category-查询板块分类)（参见 板块分类）
...
| 参数名 | 类型 | 中文名称 | 必填 | 默认值 | 参数用法说明 |
| --- | --- | --- | --- | --- | --- |
| symbol | str | 股票代码 | N | None | 输入标的代码，可输入多个. 采用 str 格式时，多个标的代码必须用英文逗号分割，如：'SHSE.600008,SZSE.000002' 采用 list 格式时，多个标的代码示例：['SHSE.600008', 'SZSE.000002'] 默认None表示所有标的。 |
| trade_date | str or datetime.date | 交易日期 | N | None | 交易日期，支持str格式（%Y-%m-%d 格式）和 datetime.date 格式，默认None表示最新交易日期。 |
| df | bool | 返回格式 | N | False | 是否返回 dataframe 格式 ， 默认False返回 list[dict],列表每项的dict的key值为参数指定的 fields 。 |
...
**2.** 订单大小具体定义：[https://finance.eastmoney.com/a/20110101117172217.html](https://finance.eastmoney.com/a/20110101117172217.html)
...
**4.** 数据最早为2025-02-21。
     ...
     [ ** ](python_select_api_stock.html#stkgetdailybasicpt---查询股本等基础指标单日截面数据（多标的）) [ ** ](python_select_api_stock_vip.html#stkgetindustrycategory---查询行业分类)
