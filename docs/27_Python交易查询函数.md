...
### context.account().position() - 查询当前账户指定持仓

**原型：**

```
context.account(account_id=None).position(symbol, side)
...
```
     [ ** ](python_trade_api.html#bondconvertibleputcancel---可转债回售撤销) [ ** ](trade_data.html#getcash---查询指定交易账户的资金信息)
