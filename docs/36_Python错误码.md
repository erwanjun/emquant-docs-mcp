...
| 1001 | 无法连接到终端服务 | 检查是否开启了东方财富量化终端 |
| 1010 | 无法获取东方财富量化服务器地址列表 | 检查是否开启了东方财富量化终端 |
| 1013 | 交易服务调用错误 | 检查终端是否正常或重启东方财富量化终端 |
| 1014 | 历史行情服务调用错误 | 在微信群或者QQ群通知技术支持 |
| 1015 | 策略服务调用错误 | 检查终端是否正常或重启东方财富量化终端 |
| 1016 | 动态参数调用错误 | 检查动态参数设置 |
| 1017 | 基本面数据服务调用错误 | 在微信群或者QQ群通知技术支持 |
| 1018 | 回测服务调用错误 | 重启东方财富量化终端、重新运行策略 |
| 1019 | 交易网关服务调用错误 | 检查终端是否正常或重启东方财富量化终端 |
...
| 2002 | 超出业务授权范围 | 调整数据日期范围，或者联系技术支持延长权限 |
| 2003 | 实时行情订阅代码数量超过用户权限 | 联系商务咨询权限 |
| 3001 | 超出数据接口调用频率限制（流控） | 检查程序是否异常运行导致循环取数，增加等待时间再调用 |

     [ ** ](python_enum_constant.html#algoorderstatus-算法单状态,暂停/恢复算法单时有效)
